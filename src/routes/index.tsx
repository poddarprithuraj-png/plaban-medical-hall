    ...
    <>
      ...
      <main>
        <section id="top" ref={heroRef} className="relative flex min-h-[570px] h-[88svh] max-h-[940px] items-end overflow-hidden bg-dark-panel text-hero-foreground">
          <motion.img src={heroPhoto.url} alt="A pharmacist thoughtfully arranging bottles in a traditional pharmacy" width={1600} height={1067} className="absolute inset-0 h-[115%] w-full object-cover object-[50%_48%]" style={reducedMotion ? undefined : { y: heroY }} initial={{ scale: 1.12, opacity: 0.4 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 1.9, ease }} />
          <div className="hero-mask absolute inset-0" />
          <motion.div className="pointer-events-none absolute right-[8%] top-[22%] hidden h-28 w-28 rounded-full border border-line-light lg:block" animate={reducedMotion ? undefined : { y: [-8, 8, -8], rotate: [0, 8, 0] }} transition={{ duration: 7, repeat: Infinity, ease: "easeInOut" }} />
          <motion.div className="pointer-events-none absolute right-[15%] top-[32%] hidden h-1.5 w-1.5 rounded-full bg-hero-foreground lg:block" animate={reducedMotion ? undefined : { y: [0, 20, 0] }} transition={{ duration: 5, repeat: Infinity }} />
        ...
        <section id="story" ref={storyRef} className="overflow-hidden bg-background py-24 md:py-36 lg:py-44">
          <div className="site-container grid grid-cols-1 gap-14 lg:grid-cols-12 lg:gap-8">
            ...
            <div className="relative lg:col-span-5 lg:col-start-8 lg:row-span-2 lg:-mt-1"><Reveal><div className="relative aspect-[1.12] overflow-hidden rounded-[4px] bg-muted"><motion.img src={detailPhoto.url} alt="Pharmacist examining a medicine bottle on a shelf" width={1600} height={1067} loading="lazy" className="absolute inset-0 h-[115%] w-full object-cover object-[50%_45%]" style={reducedMotion ? undefined : { y: storyY }} /></div><div className="absolute -bottom-7 -left-7 hidden bg-paper px-7 py-5 shadow-brand md:block"><p className="editorial-label text-muted-foreground">The person behind the care</p><p className="mt-1 font-display text-2xl">Ranit Saha</p><p className="text-sm text-ink-soft">Pharmacist</p></div></Reveal></div>
